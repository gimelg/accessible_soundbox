...
// Rotate the pouch 90° in the lid plane.
// false = long axis along Y (current behavior)
// true  = long axis along X (rotated 90°)
...
// Outer shell dimensions under the lid (unrotated naming)
...
// Effective outer footprint extents after rotation
...
// Rotate children 180° around Z about the enclosure center (keeps part in-place).
...
module lid_pouch_opening_cut() {
  ...
  translate([pouch_cx, pouch_cy, outer_hgt - lid_th - 0.2])
    linear_extrude(height=lid_th + 0.4, convexity=10)
      rotate(rot2d) capsule2d(Lc, Wc, Rc);
}
    ...
    translate([pouch_cx, pouch_cy, z_und - lid_ring_th])
      difference() {
        linear_extrude(height=lid_ring_th, convexity=10)
          rotate(rot2d) capsule2d(Lout, Wout, Rout);
